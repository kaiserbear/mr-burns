...
/*
	...
	Image Map Population and Initiation

	All global variables will be accessible in this document via app.js
...
function populateImagePreview() {
	...
	this.applyImage = function() {
		var imgSrc = imageSelect.options[imageSelect.selectedIndex].getAttribute('data-image');
	...
	imageSelect.addEventListener("change", applyImage);
}
...
// Below is an existing jQuery plugin, that was minified - which I'm disecting to see what's possible.
        ...
        d = {
            ...
            renderSVG: function(b) {
                var e = this,
                    ...
                    }), h.el && (i = h.el), "poly" == h.shape ? (i || (i = a(document.createElementNS("http://www.w3.org/2000/svg", "polygon"))), 
                    ...
                    i.attr("x", Math.min(j[0], j[2])).attr("y", Math.min(j[1], j[3])), i.attr("width", Math.abs(j[2] - j[0])).attr("height", Math.abs(j[3] - j[1]))), i && (i.attr("class", "image-mapper-shape"), i.attr("data-area-index", g), i.css(c[b].options.style), f.prepend(i), c[b].area[g].el = i)
                })
            },
            ...
            asHTML: function(b) {
            	...
            	if(c[b]) {
            		a.each(c[b].area, function(d, e) {
	                    ...
	                    mapObj[d] = {
	                    	type : c[b].area[d].shape,
	                    	mapcoordinates : f
	                   	};

	                });

	                data.ImageMap = mapObj;
            	}
...
function($) {
    ...
    $(document).on("init", function() {
        ...
        imageMapperRow = $("#image-mapper-table tbody").html();

        $(document).on("click", "button.add-row", function(b) {
            $("#image-map").imageMapper("addArea").trigger("update"), b.preventDefault()
        });

        $(document).on("update", "#image-map", function() {
            var b = $(this).imageMapper("getData"),
                c = $("#image-mapper-table"),
                d = imageMapperRow;
            $("tbody", c).html(""), $.each(b.area, function(b) {
                d = d.replace(/im\[[\d]+\]/gi, "im[" + b + "]"), $("tbody", c).append(d)
        ...
        $(document).on("update", "#image-map", function() {
            ...
            0 == c.options.src.length ? $(".toggle-content").show() : $(".toggle-content").show()
        });
        ...
        $(document).on("click", "#image-mapper-button", function() {
            var b = $("#image-map").imageMapper("asHTML");
            $("#image-mapper-output").text(b).css("whiteSpace", "pre")
        });
        ...
        var c = {
            success: function() {
                ...
                $("#image-mapper-continue", b).attr("disabled", !1),
                $(".input-group", b).addClass("has-success").removeClass("has-error").removeClass("has-warning"), 
                $(".input-group-addon > span", b).attr("class", "glyphicon glyphicon-ok")
            },
            error: function() {
                var b = a("#image-mapper-dialog");
                $("#image-mapper-continue", b).attr("disabled", "disabled"),
                $(".input-group", b).addClass("has-error").removeClass("has-success").removeClass("has-warning"), 
                $(".input-group-addon > span", b).attr("class", "glyphicon glyphicon-remove")
            }
        };
